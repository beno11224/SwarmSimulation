...
for(i = startFrom:size(allDists,1))
    ...
    for(j = 1:size(allDists,2))
        ...
        CORRECTallDistValues = allDistValues(uvec);

        if(all(isnan(CORRECTallDistValues)))
        ...
        pause(0.1)
        DistCovered = (CORRECTallDistValues(2:end) - CORRECTallDistValues(1:end-1)) .* mmPerPixel;
        TimeTaken = CORRECTallTimeValues(2:end) - CORRECTallTimeValues(1:end-1);
        % timesUsed(j,:) = allTimeValues(2:end);
        % Velocities(j,:) = DistCovered./TimeTaken;
        velocities(j,:) = interp1(CORRECTallTimeValues(2:end), DistCovered./TimeTaken, xCommon); %For Distance 
    end
