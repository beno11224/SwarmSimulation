...
for(i = startFrom:size(allDists,1))
    ...
    AvgSpread(end+1) = mean(avgVal(EndIndexes)).*100;
...
YLIMS = [0.01,0.2];
avgLine = plot(XValues(startFrom:length(XValues)), AvgSpread);
patchLine = patch([XValues(startFrom:length(XValues)) fliplr(XValues(startFrom:length(XValues)))], [(AvgSpread + StandardDeviation) fliplr((AvgSpread - StandardDeviation))],avgLine.Color, 'FaceAlpha', .3 ,'LineStyle','none');
ylabel("Percentage change in particle spread when measured as a ratio in pixels")
xlabel("Field Magnitude (s)")
