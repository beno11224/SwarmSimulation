...
class IRLClassCONV:    
    ...
    def ConstructNetwork(self, n_states, n_actions, params):
        ...
        model.add(Conv2D(params[0], activation='relu', kernel_size=5, input_shape=(n_states,n_states,1,)))
        #model.add(Activation('relu'))
        model.add(Dropout(0.2))
        #model.add(Flatten())
        model.add(MaxPooling2D(pool_size=(3, 3), strides=(1, 1), padding='valid'))
        # Second layer.
        model.add(Conv2D(params[1], activation='relu', kernel_size=5))
        #model.add(Activation('relu'))
        model.add(Dropout(0.2))
        model.add(Flatten())
        # Output layer.
...
#Convex optimisation
    ...
    def __init__(self, n_states, n_actions, params, epsilon= 0.2, lr = 0.15):
